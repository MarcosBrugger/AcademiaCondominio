...
public class UsuarioRepository {
	...
	public Usuario insereUsuario(Usuario objeto) throws Exception{
		if (objeto.ehNovo()) {
			String sql = "INSERT INTO usuario(usuario, senha, apartamento) VALUES(?, ?, ?);";
			
			PreparedStatement stmt = conn.prepareStatement(sql);
			stmt.setString(1, objeto.getUsuario());
			stmt.setString(2, objeto.getSenha());
			stmt.setString(3, objeto.getApartamento());
			stmt.execute();
			
			conn.commit();	
		}else {
			String sql = "UPDATE usuario SET usuario=?, senha=?, apartamento=? WHERE id = "+objeto.getId()+";";
